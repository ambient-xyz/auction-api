...
fn v3_post_preserves_the_existing_payload_and_instruction_gaps() {
    assert!(AuctionInstruction::try_from(22_u8).is_err());
    assert!(AuctionInstruction::try_from(28_u8).is_err());
...
#[test]
fn small_configuration_writes_preserve_other_policy_bytes() {
    let mut policy = ConfigPolicyV2::production_default();
    policy.reserved_words[0] = [11; 32];
    let before = bytemuck::bytes_of(&policy).to_vec();
